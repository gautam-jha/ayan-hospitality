...
import { getServiceBySlug, getLogisticsServices, getTestimonialById } from '@/lib/repository';
...
export default async function LogisticsServicePage({ params }: { params: Promise<{ service: string }> }) {
  const { service: slug } = await params;
  const service = await getServiceBySlug(slug);
  if (!service || service.pillar !== 'logistics') notFound();
  ...
  const logisticsServices = await getLogisticsServices();
  const related = logisticsServices.filter((s) => s.slug !== slug).slice(0, 3);
  ...
  return (
    <div className="pt-20">
      ...
      <section className="section-padding bg-cream-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="font-display text-2xl text-maroon-700 font-semibold mb-8">Other logistics services</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {related.map((s) => (
              <a key={s.slug} href={`/services/logistics/${s.slug}`} className="card-warm p-6 block group">
