...
export function BuildPackageWizard() {
  ...
  if (submitted) {
    return (
      <div className="bg-white rounded-3xl border border-cream-300 shadow-warm-lg p-10 text-center">
        ...
        <p className="text-charcoal-muted mb-4">We'll reach out to <strong>{name}</strong> within 24 hours with a detailed quote.</p>
        <div className="bg-cream-100 rounded-2xl p-6 border border-cream-300 text-left mb-8">
          <p className="text-sm font-semibold text-charcoal mb-2">Your indicative estimate</p>
          <p className="font-display text-2xl text-maroon-700 font-semibold">
            {guests && cities ? getEstimate(guests, cities, selectedServices) : ''}
          </p>
          <p className="text-charcoal-muted text-xs mt-1">Based on your selections. Final quote may vary.</p>
        </div>
        <a
          href={`https://wa.me/918826104232?text=${waMsg}`}
          target="_blank" rel="noopener noreferrer"
          className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-whatsapp text-white font-medium hover:brightness-110 transition-all"
        >
  ...
  return (
    <div className="bg-white rounded-3xl border border-cream-300 shadow-warm-lg overflow-hidden">
      {/* Progress bar */}
      ...
      <div className="p-8 lg:p-10">
              ...
              <button
                ...
                className={`p-5 rounded-2xl border-2 text-left transition-all ${
                  guests === opt.value
                    ...
                    : 'border-cream-300 hover:border-maroon-700/40'
                }`}
              >
          ...
          <div>
            {['Hospitality', 'Logistics'].map((pillar) => (
              <div key={pillar} className="mb-6">
                <h3 className="text-xs font-semibold tracking-widest uppercase text-charcoal-muted mb-3">{pillar}</h3>
                <div className="flex flex-wrap gap-2">
                  {SERVICES_LIST.filter((s) => s.pillar === pillar).map((s) => (
                    ...
                    >
                      {selectedServices.includes(s.id) && <CheckCircle2 className="w-3.5 h-3.5 inline mr-1" />}
                      {s.label}
                    </button>
                  ))}
                </div>
            ...
            {selectedServices.length > 0 && (
              <div className="mt-4 p-4 bg-cream-100 rounded-xl">
                <p className="text-sm text-charcoal-muted">Indicative range based on your selections:</p>
                <p className="font-display text-2xl text-maroon-700 font-semibold mt-1">
                  {guests && cities ? getEstimate(guests, cities, selectedServices) : '—'}
                </p>
              </div>
            )}
        ...
        {step === 4 && (
          <div className="space-y-4">
            <div className="bg-cream-100 rounded-2xl p-5 border border-cream-300 mb-6">
              ...
              <p className="text-charcoal-muted text-xs mt-1">
                {selectedServices.length} service{selectedServices.length > 1 ? 's' : ''} · {guests} guests · {cities} {parseInt(cities || '1') === 1 ? 'city' : 'cities'}
              </p>
            </div>
            <input
              type="text" placeholder="Your name *" value={name} onChange={(e) => setName(e.target.value)} required
              className="w-full px-5 py-3.5 rounded-xl border border-cream-300 focus:outline-none focus:ring-2 focus:ring-maroon-700/30 text-charcoal text-sm"
            />
            <input
              type="tel" placeholder="Phone / WhatsApp *" value={phone} onChange={(e) => setPhone(e.target.value)} required
              className="w-full px-5 py-3.5 rounded-xl border border-cream-300 focus:outline-none focus:ring-2 focus:ring-maroon-700/30 text-charcoal text-sm"
            />
          </div>
        )}
        ...
        <div className="flex items-center justify-between mt-8 pt-6 border-t border-cream-200">
          {step > 0 ? (
            <button
              ...
              className="flex items-center gap-2 text-charcoal-muted hover:text-maroon-700 transition-colors font-medium"
            >
          ...
          {step < steps.length - 1 ? (
            <button
              ...
              className="flex items-center gap-2 px-6 py-3 rounded-full bg-maroon-700 text-white font-medium hover:bg-maroon-800 disabled:opacity-40 disabled:cursor-not-allowed transition-all active:scale-95"
            >
            ...
            <button
              ...
              disabled={!canProceed()}
              id="wizard-submit"
              className="flex items-center gap-2 px-6 py-3 rounded-full bg-gold-500 text-white font-medium hover:bg-gold-400 disabled:opacity-40 disabled:cursor-not-allowed transition-all active:scale-95"
            >
